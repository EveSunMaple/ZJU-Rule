...
export function normalizeRuleSource(source, origin) {
  ...
  // subconverter 的本地规则路径约定：
  //   rules/ZJU-Rule/Clash/ZJU.list
  // 其中 rules/ZJU-Rule/ 就是本仓库的一次 checkout，所以剥掉前缀即可。
  // 仓库里 ACL4SSR_*.ini（不带 _Online_ 的那些）用的就是这种写法。
  const local = /^rules\/ZJU-Rule\/(.+)$/.exec(s);
  if (local) return `/${local[1]}`;

  // 直接以仓库顶层目录开头的相对路径
  if (/^(Clash|configs)\//.test(s)) return `/${s}`;
