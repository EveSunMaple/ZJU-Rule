...
/**
 * Mihomo（Clash Meta）实际支持的规则类型。
 *
 * 这张表是**用真的 mihomo 二进制逐个探测出来的**，不要凭印象改：
 *   USER-AGENT  不被支持（Clash Premium 时代的老写法）
 *   URL-REGEX   不被支持
 *   UID         只在移动端存在
 *
 * 规则文件里混着这些类型（ACL4SSR 上游是给 Surge / Clash Premium 写的），
 * 直接输出会让 Clash 拒绝加载整份配置 —— 也就是「订阅配置校验失败」。
 * subconverter 也是这么做的：它生成的 Clash 配置里同样过滤掉了这两类规则。
 *
 * 探测方法见 tools/probe-rule-types.mjs，校验逻辑见 tools/test-mihomo.mjs。
 */
 ...
 *
 * @param {string[]} rules
 * @returns {{ rules: string[], dropped: Array<{type: string, count: number, samples: string[]}> }}
 */
export function filterUnsupportedRules(rules) {
  const kept = [];
  const stats = new Map();

  for (const rule of rules) {
    const type = String(rule).split(',')[0].trim().toUpperCase();
    if (MIHOMO_RULE_TYPES.has(type)) {
      kept.push(rule);
      continue;
    }
    if (!stats.has(type)) stats.set(type, { type, count: 0, samples: [] });
    const entry = stats.get(type);
    entry.count += 1;
    if (entry.samples.length < 2) entry.samples.push(rule);
  }

  return { rules: kept, dropped: [...stats.values()] };
}
